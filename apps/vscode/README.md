...
The image runs as `65534:65534` (`nobody:nogroup`) by default and only uses the
paths you mount into the container:

- `/config` for code-server user data and installed extensions
- `/workspace` for the files you want to edit
...
Do not mount the host root or broad host paths into `/workspace`. In Kubernetes,
prefer a PVC or a narrow repository checkout volume.
